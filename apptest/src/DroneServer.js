 ...
 * ESP-Drone 服务端核心
 * ==========================
 * 负责 TCP/HTTP/WebSocket/UDP 的统一管理
 */
...
class DroneServer {
    constructor() {
        ...
        this.clientsByIP = new Map();      // key: droneIP，用于匹配 UDP 遥测来源
    ...
    ensureConnectedClients(res) {
        if (this.hasConnectedClients()) {
            return true;
        }

        res.status(400).json({ error: 'No drone connected' });
        return false;
    }

    forEachDroneClient(callback) {
    ...
    applyPidParamToAllClients(param) {
        ...
        this.forEachDroneClient((client) => {
            try {
                this.applyPidParamToClient(client, param);
                appliedClients++;
    ...
    startDiscoveryBroadcast() {
        try {
            ...
            this.discoverySocket.on('message', (msg, rinfo) => {
                ...
                // TCP 端口为 0 时视为发现请求，使用单播返回
                // 返回格式: [4字节 ESPD] + [2字节 TCP端口] + [2字节 UDP遥测端口]
                if (reqPort === 0) {
            ...
            this.discoverySocket.bind(CONFIG.DISCOVERY_PORT, () => {
                ...
                console.log(`[Discovery] UDP broadcast on port ${CONFIG.DISCOVERY_PORT}`);
                ...
                // 广播包格式: [4字节 ESPD] + [2字节 TCP端口] + [2字节 UDP遥测端口]
    ...
    /**
     * 启动 vofa+ UDP 转发服务
     * 接收 vofa+ 客户端注册请求，并将遥测数据通过 FireWater 协议转发出去
     */
    startVofaServer() {
        try {
    ...
    startUDPTelemetryServer() {
        try {
            ...
            this.udpTelemetrySocket.bind(CONFIG.UDP_TELEMETRY_PORT, () => {
                ...
                console.log('  飞控通过该端口发送遥测包（姿态角/电机/陀螺仪），释放 TCP 带宽');
            });
        } catch (err) {
    ...
    startWebServer() {
        ...
        this.httpServer.listen(CONFIG.HTTP_PORT, () => {
            console.log(`[HTTP] HTTP/WebSocket server listening on port ${CONFIG.HTTP_PORT}`);
    ...
    setupAPIRoutes(app) {
        ...
        app.post('/api/control', (req, res) => {
            ...
            if (!this.ensureConnectedClients(res)) {
                return;
            }
        ...
        app.post('/api/motor/set', (req, res) => {
            ...
            this.sendParamSetToAllClients('motortest', 'id', PARAM_UINT8, safeMotorId, 1, (client) => {
                client.handleMotorCommand(safeMotorId, safeThrust);
            });
            this.sendParamSetToAllClients('motortest', 'thrust', PARAM_UINT16, safeThrust, 2);
            this.sendParamSetToAllClients('motortest', 'mode', PARAM_UINT8, 2, 1);
            res.json({ success: true });
        });
        ...
        app.post('/api/motor/estop', (req, res) => {
            ...
            this.sendParamSetToAllClients('motortest', 'estop', PARAM_UINT8, 1, 1);
            this.sendParamSetToAllClients('motortest', 'mode', PARAM_UINT8, 0, 1);
            res.json({ success: true });
        });
        ...
        app.post('/api/motor/all', (req, res) => {
            ...
            const safeThrust = Math.max(0, Math.min(65535, Number(thrust) || 0));
            this.sendParamSetToAllClients('motortest', 'thrust', PARAM_UINT16, safeThrust, 2);
            this.sendParamSetToAllClients('motortest', 'mode', PARAM_UINT8, 3, 1);
            res.json({ success: true });
        });
        ...
        app.post('/api/motor/sequential', (req, res) => {
            ...
            this.sendParamSetToAllClients('motortest', 'mode', PARAM_UINT8, 1, 1);
            res.json({ success: true });
        });

        // ESC 首次行程校准（触发参数 escCalib.request=1/2/3）
        app.post('/api/esc/calibrate-first', (req, res) => {
            ...
            try {
                ...
                this.sendParamSetToAllClients('escCalib', 'request', PARAM_UINT8, cmd, 1);
        ...
        app.post('/api/vofa/add-client', (req, res) => {
            ...
            const clientInfo = { ip, port: clientPort };
            this.vofaClients.set(key, clientInfo);
        ...
        app.post('/api/pid/set', (req, res) => {
            ...
            const { appliedClients, results } = this.applyPidParamToAllClients(normalized);
        ...
        app.post('/api/pid/batch', (req, res) => {
            ...
            for (const p of normalizedParams) {
                ...
                let applyOk = true;
                let applyErr = '';
                const applyResults = this.applyPidParamToAllClients(p);
                for (const item of applyResults.results) {
                    if (item.error) {
                        applyOk = false;
                        applyErr = item.error;
                        break;
    ...
    removeClient(client, reason = null) {
        ...
        this.broadcastToWebClients(evt);
    }
    ...
    onTelemetry(client, telemetry) {
        ...
        if (!client.lastTelemetryLog || now - client.lastTelemetryLog >= 1000) {
            ...
            console.log(`[${client.remoteAddr}] Telemetry: ` +
                ...
                `Yaw=${telemetry.yaw.toFixed(1)}deg ` +
    ...
    broadcastTelemetry(addr, telemetry) {
        ...
        // 同时转发到 vofa+ (FireWater 协议)，保持全速
        if (this.vofaEnabled && this.vofaSocket && this.vofaClients.size > 0) {
            this.sendToVofaClients(telemetry);
        }

        const webInterval = Math.max(1, Number(CONFIG.WEB_TELEMETRY_INTERVAL) || 10);
        const lastAt = this.lastWebTelemetryAt.get(addr) || 0;
        if (now - lastAt < webInterval) {
            return;
        }
        this.lastWebTelemetryAt.set(addr, now);
    ...
    /**
     * 通过 FireWater 协议转发遥测数据到 vofa+
     * 格式: channel1,channel2,...,channel13\n
     */
    sendToVofaClients(telemetry) {
        if (!telemetry) return;

        // 构建 FireWater 格式字符串，使用逗号分隔浮点数
        const values = [
            telemetry.roll || 0,
            telemetry.pitch || 0,
            telemetry.yaw || 0,
        ...
        // FireWater 格式: 13 个浮点数，逗号分隔，末尾换行
