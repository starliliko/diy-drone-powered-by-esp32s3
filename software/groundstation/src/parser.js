...
function parseTelemetry(buffer) {
    ...
    if (buffer.length >= 52) {
        ...
        // V3.4 新增磁力计数据 (93字节)
        if (buffer.length >= 93) {
            // 磁力计 (milli-gauss -> gauss)
            telemetry.magX = buffer.readInt16LE(offset + 85) / 1000.0;
            telemetry.magY = buffer.readInt16LE(offset + 87) / 1000.0;
            telemetry.magZ = buffer.readInt16LE(offset + 89) / 1000.0;
            // 磁航向 (centi-deg -> deg)
            telemetry.magHeading = buffer.readInt16LE(offset + 91) / 100.0;
        } else {
            telemetry.magX = 0;
            telemetry.magY = 0;
            telemetry.magZ = 0;
            telemetry.magHeading = 0;
        }
