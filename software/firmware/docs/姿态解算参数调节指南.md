...
### 当前系统配置
- **IMU**: BMI088 (SPI接口)
- **气压计**: MS5611 (驱动启用，Kalman中默认不参与更新)
- **控制频率**: 1000Hz
- **Kalman预测频率**: 500Hz（`PREDICT_RATE`）
- **加速度姿态校正**: 25Hz（`ACCEL_ATTITUDE_RATE`）
...
| 参数名                    | 当前值   | 单位  | 说明                       | 调节建议                                                                                |
| ------------------------- | -------- | ----- | -------------------------- | --------------------------------------------------------------------------------------- |
| `measNoiseGyro_rollpitch` | **0.15f** | rad/s | 姿态误差过程噪声（Roll/Pitch） | 增大→更快收敛但更敏感<br>减小→更平滑但收敛慢<br>推荐范围: 0.05 ~ 0.5 |
| `measNoiseGyro_yaw`       | **0.1f**  | rad/s | 姿态误差过程噪声（Yaw）        | 增大→Yaw允许更快变化<br>减小→Yaw更平稳但可能更慢纠正                                   |
| `measNoiseBaro`           | 12.0f     | m     | 气压计测量噪声               | 仅在开启 `KALMAN_USE_BARO_UPDATE` 时生效                                                |

> **💡 关键理解**:
> - 这里的 `mNGyro_*` 在当前实现中用于姿态误差协方差增长速率（过程噪声项）
> - 值越大，滤波器对姿态误差变化更“敏感”；值越小，姿态估计更“稳”但可能纠正更慢
> - 当前默认值（0.15/0.1）是面向 BMI088 的工程折中
...
| 参数名            | 当前值 | 单位 | 说明                 |
| ----------------- | ------ | ---- | -------------------- |
| `procNoiseAcc_xy` | 2.0f   | m/s² | 加速度计XY轴过程噪声 |
