...
typedef enum
{
  ArmThrottleStateDisarmed = 0,
  ArmThrottleStateWaitLowThrottle,
  ArmThrottleStateReady,
} ArmThrottleState;

static ArmThrottleState armThrottleState = ArmThrottleStateDisarmed;
static uint32_t armThrottleWarnCounter = 0;
static uint32_t armThrottleLowHoldCounter = 0;

#define ARM_THROTTLE_LOW_THRESHOLD 3000
#define ARM_THROTTLE_WARN_PERIOD_TICKS 500
#define ARM_THROTTLE_LOW_HOLD_TICKS 100
...
static void stabilizerTask(void *param)
{
  ...
  DEBUG_PRINTI("Flight Mode: %s", vehicleGetFlightModeName(vehicleGetFlightMode()));
...
*/
      ...
      const bool isArmedNow = vehicleIsArmed();
      {

        if (!isArmedNow)
        {
          armThrottleState = ArmThrottleStateDisarmed;
          armThrottleWarnCounter = 0;
          armThrottleLowHoldCounter = 0;
        }
        else
        {
          if (armThrottleState == ArmThrottleStateDisarmed)
          {
            armThrottleState = ArmThrottleStateWaitLowThrottle;
            armThrottleWarnCounter = 0;
            armThrottleLowHoldCounter = 0;
            ESP_LOGW("STAB", "ARM safety: waiting for low throttle hold confirmation");
          }

          if (armThrottleState == ArmThrottleStateWaitLowThrottle)
          {
            if (control.thrust < ARM_THROTTLE_LOW_THRESHOLD)
            {
              if (armThrottleLowHoldCounter < ARM_THROTTLE_LOW_HOLD_TICKS)
              {
                armThrottleLowHoldCounter++;
              }

              if (armThrottleLowHoldCounter >= ARM_THROTTLE_LOW_HOLD_TICKS)
              {
                armThrottleState = ArmThrottleStateReady;
                ESP_LOGI("STAB", "ARM safety: low throttle confirmed, motor output enabled");
              }
            }
            else
            {
              ...
              if (++armThrottleWarnCounter >= ARM_THROTTLE_WARN_PERIOD_TICKS)
              {
                armThrottleWarnCounter = 0;
                ESP_LOGW("STAB", "High throttle detected right after arming, output blocked until throttle goes low");
              }
            }
          }
        }
      ...
      {
        checkStops = isArmedNow;
        if (emergencyStop || !isArmedNow || armThrottleState != ArmThrottleStateReady)
        {
          powerStop(); // 停机
        }
        else
...
{
  return (armThrottleState != ArmThrottleStateReady) && vehicleIsArmed();
}
