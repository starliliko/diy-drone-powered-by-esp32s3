...
static bool armThrottleReady = false;
static bool armThrottleWasArmed = false;
...
*/
      ...
      // 电机测试模式时，stabilizer 完全不控制电机，让 motor_test 任务独占
      {
        ...
        if (!isArmedNow)
        {
          armThrottleReady = false;
          armThrottleWasArmed = false;
          armThrottleWarnCounter = 0;
          armThrottleLowHoldCounter = 0;
        }
        else
        {
          if (!armThrottleWasArmed)
          {
            armThrottleReady = false;
            armThrottleWarnCounter = 0;
            armThrottleLowHoldCounter = 0;
      ...
      else
      {
        ...
        if (emergencyStop || !vehicleIsArmed() || !armThrottleReady)
        {
          powerStop(); // 停机
        }
        else
...
{
  return (!armThrottleReady) && vehicleIsArmed();
}
