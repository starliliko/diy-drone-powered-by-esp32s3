...
static float procNoiseAcc_xy = 2.0f; // 增大到2.0，让积分预测不确定性增长更快（降低积分信任度）
...
// MTF01: 测量标准差（越小权重越高）
// 光流权重约80%：procNoiseAcc_xy=2.0, flowStdDev=0.005
float flowStdDev = 0.05f; // 减小到0.005，提高光流权重到
float tofStdDev = 0.05f;  // 减小到0.05m，提高TOF权重
...
// Process noise for attitude error states D0/D1 (roll/pitch).
...
void kalmanCoreUpdateWithAccel(kalmanCoreData_t *this, const Axis3f *acc, float stdDev)
{
  ...
  // Tight threshold [0.9, 1.1]g: max 0.1g lateral accel allowed (~5.7 deg tilt error at most)
  float accMag = sqrtf(acc->x * acc->x + acc->y * acc->y + acc->z * acc->z);
  if (accMag < 0.9f || accMag > 1.1f)
  {
  ...
  // Note: yaw (D2) cannot be corrected by accelerometer (gravity has no yaw information).
  // Yaw drift prevention is handled separately in estimator_kalman.c via the yaw anchor mechanism.
}
...
void kalmanCorePredict(kalmanCoreData_t *this, float cmdThrust, Axis3f *acc, Axis3f *gyro, float dt, bool quadIsFlying)
{
  ...
  // LMS Adaptive Cross-Axis Compensation for Gyro Z
  // ================================================
  // BMI088 cross-axis sensitivity (~1-2%) causes gx/gy to leak into gz,
  // which integrates into permanent yaw drift during pitch/roll motion.
  //
  // Model: gz_error = kxz*gx + kyz*gy + bz
  //
  // Error signal: EULER yaw rate (not world-frame ω_z!).
  //   ψ̇·cos²θ = ωy·R[2][1] + ωz·R[2][2]
  //
  // CRITICAL FIX: The old error signal ω_z_world = R[2][0]·ωx + R[2][1]·ωy + R[2][2]·ωz
  // is WRONG because during pure roll motion at pitch tilt θ:
  //   ω_z_world = -sin(θ)·φ̇  ≠ 0  (spurious!)
  // This caused the LMS to learn incorrect coefficients, WORSENING drift.
  //
  // The Euler yaw rate ψ̇ = (ωy·sinφ + ωz·cosφ)/cosθ is exactly zero
  // for ANY pure roll or pitch motion, regardless of tilt angle.
  // Using ψ̇·cos²θ = ωy·R[2][1] + ωz·R[2][2] avoids the cosθ division.
  //
  // LMS gradient: ∂e/∂kxz = -ωx·R22, ∂e/∂kyz = -ωy·R22
  //   → kxz += μ·e·ωx·R22,  kyz += μ·e·ωy·R22
  static float kxz = 0.0f; // gz error per unit gx (cross-axis coeff)
  static float kyz = 0.0f; // gz error per unit gy (cross-axis coeff)
  static float bz = 0.0f;  // gz constant bias (rad/s)

  // Apply correction always (hardware error exists in flight too)
  float gz_corrected = gyro->z - kxz * gyro->x - kyz * gyro->y - bz;

  // Learn coefficients only on ground (Euler yaw rate ≈ 0)
  if (!quadIsFlying)
  {
    float R21 = this->R[2][1]; // cos(θ)·sin(φ)
    float R22 = this->R[2][2]; // cos(θ)·cos(φ)
    if (fabsf(R22) > 0.5f)     // tilt < ~60°, projection reliable
    {
      // Euler yaw rate error (proportional to ψ̇·cos²θ)
      // Exactly zero for pure roll or pitch motion at any tilt angle
      float yawRateErr = gyro->y * R21 + gz_corrected * R22;

      // Gate: reject deliberate yaw rotation
      if (fabsf(yawRateErr) < 0.3f) // < ~17°/s
      {
        // LMS gradient descent on yawRateErr²
        //   μ = 5e-4: with E[ω²]≈4 at 500Hz → τ ≈ 1s convergence
        //   μ_bias = 5e-5: 10× slower (thermal drift changes slowly)
        float mu = 5e-4f;
        kxz += mu * yawRateErr * gyro->x * R22;
        kyz += mu * yawRateErr * gyro->y * R22;
        bz += 5e-5f * yawRateErr * R22;

        // Clamp to physical limits (max 5% cross-axis, ~3°/s bias)
        if (kxz > 0.05f)
          kxz = 0.05f;
        else if (kxz < -0.05f)
          kxz = -0.05f;
  ...
  float dtwz = dt * gz_corrected;

  // compute the quaternion values in [w,x,y,z] order
  float angle = xtensa_sqrt(dtwx * dtwx + dtwy * dtwy + dtwz * dtwz + 1e-10f); // 防止除零
  ...
  float norm_sq = tmpq0 * tmpq0 + tmpq1 * tmpq1 + tmpq2 * tmpq2 + tmpq3 * tmpq3;
  if (norm_sq < 1e-10f)
  {
    // 四元数退化，重置为单位四元数
    this->q[0] = 1.0f;
  ...
  {
    float norm = xtensa_sqrt(norm_sq);
    this->q[0] = tmpq0 / norm;
    this->q[1] = tmpq1 / norm;
    this->q[2] = tmpq2 / norm;
    this->q[3] = tmpq3 / norm;
