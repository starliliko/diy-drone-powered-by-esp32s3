...
#define DEBUG_MODULE "ESPNOW_CTRL"
#include "debug_cf.h"
...
static bool espnow_ctrl_responder_is_bindlist(const uint8_t *mac, espnow_attribute_t initiator_attribute)
{
    ...
    {
        if (!memcmp(g_bindlist.data[i].mac, mac, 6) && g_bindlist.data[i].initiator_attribute == initiator_attribute)
        {
            return true;
        }
    }
                                                    ...
                                                    size_t size, wifi_pkt_rx_ctrl_t *rx_ctrl)
{
    ...
    {
        DEBUG_PRINT("bind, esp_log_timestamp: %d, timestamp: %d, rssi: %d, rssi: %d\n",
        ...
        {
            DEBUG_PRINT("addr: " MACSTR ", initiator_type: %d, initiator_value: %d\n",
    ...
    else
    {
        for (int i = 0; i < g_bindlist.size; ++i)
        {
            if (!memcmp(g_bindlist.data[i].mac, src_addr, 6) && g_bindlist.data[i].initiator_attribute == ctrl_data->initiator_attribute)
            {
                                                    ...
                                                    size_t size, wifi_pkt_rx_ctrl_t *rx_ctrl)
{
    ...
    espnow_ctrl_data_t *ctrl_data = (espnow_ctrl_data_t *)data;
    DEBUG_PRINT_LOCAL("src_addr: " MACSTR ", espnow_ctrl_responder_recv, value: s1 = %d, s2 = %d, lx = %d, ly = %d, rx = %d, ry = %d, ch1 = %d, ch2 = %d\n",
                                           ...
                                           size_t size, wifi_pkt_rx_ctrl_t *rx_ctrl)
{
    ...
    DEBUG_PRINT("src_addr: " MACSTR ", %s, channel: %d\n", MAC2STR(src_addr), __func__, channel);
